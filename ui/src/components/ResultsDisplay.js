...
const ResultsDisplay = ({ result, isStreaming = false, onProfileSaved }) => {
  ...
  const handleSaveProfile = async () => {
    // Don't allow saving while streaming
  ...
  return (    
    ...
    >
      ...
      <Card variant="compact" className="shadow border border-slate-200 flex-shrink-0">
        <div className="flex justify-between items-start mb-3">
          <h3 className="text-xl font-semibold text-gray-800 flex items-center">
            ...
            Executive Profile Summary
            {isStreaming && !result.aggregated_profile && (
              <div className="ml-2 w-4 h-4 border-2 border-blue-200 border-t-blue-600 rounded-full animate-spin"></div>
            )}
          </h3>
        ...
        {/* Executive Profile Content with proper height constraints */}
            ...
            <AnimatePresence mode="wait">
              {isStreaming && !result.aggregated_profile ? (
                ...
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.5 }}
                  className="prose prose-slate prose-headings:mb-3 prose-p:mb-2 prose-ul:mb-2"
                >
                  <ReactMarkdown
                    components={{
                      h1: ({node, children, ...props}) => 
                        children ? <h1 className="text-2xl font-bold mb-4 text-gray-900" {...props}>{children}</h1> : null,
                      h2: ({node, children, ...props}) => 
                        children ? <h2 className="text-xl font-semibold mb-3 text-gray-800" {...props}>{children}</h2> : null,
                      h3: ({node, children, ...props}) => 
                        children ? <h3 className="text-lg font-semibold mb-2 text-gray-800" {...props}>{children}</h3> : null,
                      p: ({node, children, ...props}) => 
                        <p className="mb-3 text-gray-600 leading-relaxed" {...props}>{children}</p>,
                      ul: ({node, children, ...props}) => 
                        <ul className="list-disc pl-5 mb-3 space-y-1 text-gray-600" {...props}>{children}</ul>,
                      ol: ({node, children, ...props}) => 
                        <ol className="list-decimal pl-5 mb-3 space-y-1 text-gray-600" {...props}>{children}</ol>,
      ...
      {/* Streaming Insights Section - with flex-shrink-0 to prevent squashing */}
            ...
            <AccordionContent>
              <div className="flex flex-col gap-4 mt-2">
                <StreamingSection
                  ...
                  content={result.background_info}
                  isLoading={isStreaming && !result.background_info}
                  icon="🎓"
                />
                
                <StreamingSection
                  title="Leadership Profile"
                  content={result.leadership_info}
                  isLoading={isStreaming && !result.leadership_info}
                  icon="👑"
                />
                
                <StreamingSection
                  title="Market Reputation"
                  content={result.reputation_info}
                  isLoading={isStreaming && !result.reputation_info}
                  icon="⭐"
                />
                
                <StreamingSection
                  title="Strategic Approach"
                  content={result.strategy_info}
                  isLoading={isStreaming && !result.strategy_info}
                  icon="🎯"
                />
              </div>
            </AccordionContent>
